matriz_procesos = [[0,3,2,1,2,4,10], [2,3,2,1,2,4,30], [3,3,0,0,2,4,15]]
matriz_resultados = []
fila_resultado = []
cola_memoria = []
cola_listos = []
...
def fcfs(esquema_particiones, alg_particiones):
    ...
    while len(lista_completados) != len(matriz_procesos):
        ...
        for i in range(len(cola_listos)):
            e_listo += str(cola_listos[i][0]) + ', '
        for i in range(len(cola_entrada)):
            e_bloqueado += str(cola_entrada[i]) + ', '
        for i in range(len(cola_salida)):
            e_bloqueado += str(cola_salida[i]) + ', '

        if (salida == -1) and (len(cola_salida) > 0): #Si la salida está libre y hay procesos en la cola de salida, le asigno la salida al primero y lo saco de la cola de salida
            salida = cola_salida[0]
            salida_aux = salida
            cola_salida.pop(0)
        if salida != -1: # Si la salida está asignada a un proceso, descuento 1 al tiempo de salida restante
        ...
        if (entrada == -1) and (len(cola_entrada) > 0): #Si la entrada está libre y hay procesos en la cola de entrada, le asigno la entrada al primero y lo saco de la cola de entrada
            entrada = cola_entrada[0]
            entrada_aux = entrada
            cola_entrada.pop(0)
        if entrada != -1: # Si la entrada está asignada a un proceso, descuento 1 al tiempo de entrada restante
        ...
        if (cpu == -1) and (len(cola_listos) > 0):  #Si la cpu está libre y hay procesos en la cola de listos, le asigno la cpu al primero y lo saco de la cola de listos
            cpu = cola_listos[0][0]
            cpu_aux = cpu
            casilla_cpu = cola_listos[0][1]
            cola_listos.pop(0)
        if cpu != -1: #Si la cpu está asignada a un proceso, descuento 1 al tiempo de cpu restante
        ...
        matriz_resultados.append([tiempo, e_listo[:-2], e_bloqueado[:-2], cpu_aux, c_listo[:-2], c_entrada[:-2], c_salida[:-2], cpu_aux, entrada_aux, salida_aux])
        tiempo += 1
    print(matriz_resultados[tiempo-1])
